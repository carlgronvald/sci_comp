...
[X2,T2] = RK4StepDoubling(x0, @vanderpolf, 0.5, 0, 40, 0.01, 0.01, parameters);
...
[X2,T2] = RK4StepDoubling(x0, @vanderpolf, 0.1, 0, 40, 0.01, 0.01, parameters);
[Tcorrect, Xcorrect] = ode15s(vanmu1p5, [0 40], [1.0;1.0]);
hold off
plot(T1, X1(:,1))
hold on
...
[X2,T2] = RK4StepDoubling(x0, @CSTRf, 0.1, 0, 200, 0.01, 0.01, parameters);
...
[X1,T1] = RK4FixedStepSize(x0, @CSTR1Df, 20, 0, 200, parameters);
[X2,T2] = RK4StepDoubling(x0, @CSTR1Df, 20, 0, 200, 0.01, 0.01, parameters);
[Tcorrect, Xcorrect] = ode15s(parmcstr, [0 200], x0);
